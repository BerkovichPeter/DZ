...
int copyFile(const char* src, const char *dst) {
    ...
    if(readCount == -1 || readCount != srcSz) {
        //ошибка при чтении данных из дескриптора
        printf(stderr, "error read file descriptor src");
        close(srcFD);
        close(dstFD);
        return -1;
    }
    ssize_t writeCount = write(dstFD, buff, readCount);
    if(writeCount == -1 || writeCount != readCount) {
        //ошибка при записи данных в дескриптор
        printf(stderr, "error write file descriptor src");
        close(srcFD);
        close(dstFD);
        return -1;
    }
...
//для примера зададим смещение 2, чтобы копировалось все кроме первых 2 символов
...
int copyFile2(const char* src, const char *dst) {
    ...
    if(readCount == -1 || readCount != srcSz - 2) {
        //ошибка при чтении данных из дескриптора
        printf(stderr, "error read file descriptor src");
        close(srcFD);
        close(dstFD);
    ...
    ssize_t writeCount = pwrite(dstFD, buff, readCount, dstStat.st_size);
    if(writeCount == -1 || writeCount != readCount) {
        //ошибка при записи данных в дескриптор
        printf(stderr, "error write file descriptor src");
        close(srcFD);
