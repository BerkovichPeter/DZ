...
char *getFullName(const char *dir) {
    ...
    if(dir[0] == '~') {
        ...
        char *res = joinPath(usrName, dir[0] + 2);
        free(usrName);
        return res;
    }
...
int printDir_(const char* dirName, int offset) {
    ...
    char *offsetStr = (char*)malloc((offset * 4 + 1) * sizeof(char));
    memset(offsetStr, ' ', offset * 4);
    ...
    for(struct dirent *d = readdir(dir); d; d = readdir(dir)) {
        ...
        if(d->d_type == DT_DIR) {
            ...
            if(printDir_(joinPath1(dirName, d->d_name, d->d_namlen), offset + 1)) {
            ...
            printf("unknown: ");
            struct stat s;
            char *joined = joinPath1(dirName, d->d_namlen, d->d_name);
            stat(joined, &s);
